...
contract StringBytesContract {

    // Static byte arrays, Both declaration will create array with 3 bytes elements
    byte[3] fixedByteArray;
    // The type byte[] is an array of bytes, but due to padding rules, it wastes 31 bytes of space for each element (except in storage). 
    ...
    function conversionTest() public pure returns(string memory) {
        bytes   memory string2 = "AgamShah"; // dynamic memory bytes type
        string  memory converted = string(string2);
        return converted;
    }

    // Retrieves the element at specified index - Cannot do with strings, hence converting to bytes
    ...
    function testing() public pure {
        // uint8 need to be explicitly converted to byte type
        // Converting to byte type, since fixedByteArray is a byte type array
        // From solidity 0.5 the int to byte conversion gives you error, you can use https://github.com/pouladzade/Seriality library for that
        //fixedByteArray = [byte(1),2,3];
        
        // Assignment NOT allowed as bytes3Array is a static Array, It is readonly
        // bytes3Array[0] = 1;
